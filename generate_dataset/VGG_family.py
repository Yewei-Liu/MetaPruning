...
class MyVGG(nn.Module):
    ...
    def __init__(self, node_num, cfg=cfg['E'], batch_norm=True):
        ...
        self.idx = 1
        self.block0 = self._make_layers(len(cfg[0]), batch_norm)
        self.block1 = self._make_layers(len(cfg[1]), batch_norm)
        self.block2 = self._make_layers(len(cfg[2]), batch_norm)
        self.block3 = self._make_layers(len(cfg[3]), batch_norm)
        self.block4 = self._make_layers(len(cfg[4]), batch_norm)
    ...
    def _make_layers(self, n, batch_norm=False):
        layers = []
        for v in range(n):
            conv2d = nn.Conv2d(self.node_num[self.idx - 1], self.node_num[self.idx], kernel_size=3, padding=1)
            if batch_norm:
                layers += [conv2d, nn.BatchNorm2d(self.node_num[self.idx]), nn.ReLU(inplace=True)]
            else:
                layers += [conv2d, nn.ReLU(inplace=True)]
            self.idx += 1
        layers = layers[:-1]
