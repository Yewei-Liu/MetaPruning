...
def main(cfg):
    ...
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    metanetwork_dir = os.path.join('metanetwork', f'{cfg.model_name}_on_{cfg.dataset_name}')
    model = torch.load(f'{cfg.dataset_name}_after_train.pth')
    model = progressive_pruning(model, cfg.dataset_name, train_loader, test_loader, cfg.initial_speed_up, cfg.method, log=True)[1]
    train(model, train_loader, test_loader, 80, 0.01, "40, 70", log=True)
    torch.save(model, f'{cfg.dataset_name}_after_finetune.pth')
    visualize_acc_speed_up_curve(model, cfg.dataset_name, 'origin', test_loader, 1.0, 10.0, cfg.method, save_dir='./', name=f'{cfg.dataset_name}.png')
