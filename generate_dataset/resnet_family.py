...
class MyResNet(nn.Module):
    def __init__(self, depth, node_num, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(3, node_num[1], kernel_size=3, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(node_num[1])
        self.relu = nn.ReLU(inplace=True)
        self.node_num = node_num
        self.idx = 1
        self.layer1 = self._make_layer(n)
    ...
    def _make_layer(self, blocks, stride=1):
        downsample = None
        if stride != 1:
            downsample = nn.Sequential(
                nn.Conv2d(self.node_num[self.idx], self.node_num[self.idx + 2],
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(self.node_num[self.idx + 2]),
            )

        layers = list([])
        layers.append(MyBasicBlock(self.node_num[self.idx], self.node_num[self.idx + 1], self.node_num[self.idx + 2], stride, downsample))
        self.idx += 2
        for i in range(1, blocks):
            layers.append(MyBasicBlock(self.node_num[self.idx], self.node_num[self.idx + 1], self.node_num[self.idx + 2]))
            self.idx += 2
