...
def train(
    ...
    return_best=False,
    opt='sgd',
    freeze_zero=False,          # <--- NEW FLAG
):
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    model.eval().to(device)

    # --- build masks for zero weights (conv & linear) ---
    zero_weight_masks = {}       # key: id(param), value: mask tensor (1 for non-zero, 0 for zero)
    if freeze_zero:
        for module in model.modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                w = module.weight.data
                mask = (w != 0).float().to(device)
                zero_weight_masks[id(module.weight)] = mask
    ...
    if log:
        ...
        val_acc, val_loss = eval(model, test_loader, device)
        logger.info(
            "Before training : Train Acc=%.4f  Val Acc=%.4f" % (train_acc, val_acc)
        )
    ...
    for epoch in range(epochs):
        ...
        for i, (data, target) in enumerate(train_loader):
            ...
            # your explicit grad zeroing (kept as in original)
            for param in model.parameters():
                if param.requires_grad:
                    param.grad = torch.zeros_like(param)
            ...
            # --- apply masks to gradients to freeze zero weights ---
            if freeze_zero:
                for module in model.modules():
                    if isinstance(module, (nn.Conv2d, nn.Linear)):
                        w = module.weight
                        if w.grad is not None and id(w) in zero_weight_masks:
                            w.grad.mul_(zero_weight_masks[id(w)])

            optimizer.step()

            # --- assert all originally-zero weights remain zero ---
            if freeze_zero:
                for module in model.modules():
                    if isinstance(module, (nn.Conv2d, nn.Linear)):
                        w = module.weight.data
                        mask = zero_weight_masks.get(id(module.weight))
                        if mask is not None:
                            # check: positions where mask == 0 must still have value 0
                            if not torch.all((w * (1 - mask)) == 0):
                                raise AssertionError(
                                    f"Zero-frozen weights changed in module {module.__class__.__name__}"
                                )
    ...
    if log:
        logger.info(
            "After training : Train Acc=%.4f  Val Acc=%.4f" % (train_acc, val_acc)
        )
    return train_acc, train_loss, val_acc, val_loss
